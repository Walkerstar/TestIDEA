...
/**
 * 有一个 m x n 的二元网格，其中 1 表示砖块，0 表示空白。砖块 稳定（不会掉落）的前提是：<p>
 * 一块砖直接连接到网格的顶部，或者<p>
...
public class leetCode803 {
    ...
    public int[] hitBricks(int[][] grid, int[][] hits) {
        ...
        //第 1 步: 把 grid 中的砖头全部击碎（通常算法问题不能修改输入的数据，这一步非必需，可以认为是一种答题规范）
        ...
        //第 2 步:建图，把砖头和砖块的连接关系输入并查集，size 表示二维网格的大小，也表示虚拟的 【屋顶】 在并查集中的编号
        int size = rows * cols;
        UnionFind unionFind = new UnionFind(size);

        //将下标为 0 的这一行的砖与 【屋顶】 相连
        ...
        //其余网格，如果是砖块，向上、向左看一下，如果也是砖块，在并查集中进行合并
    ...
    private class UnionFind {
        ...
        /**
         * 路径压缩，只要求每个不相交集合的【根节点】 的子树包含的节点总数数值正确即可，
