...
(function (root, doc, factory) {
    //根据加载的方式不同，载入jQuery（根据你的require命名自定义修改）
    ...
    $.fn.esign = function (canvasEdit, sign_show, sign_clear, sign_ok) {
        ...
        //初始化相关的DOM,获得结果寄存命名空间
        let domMap = domInit(canvasEdit, sign_show, sign_clear, sign_ok);
        //获取画板引用
        let canvas = domMap.canvasEdit[0];
        //注册相关事件
        ...
        function domInit(canvasEdit, sign_show, sign_clear, sign_ok) {
            ...
            if (!sign_show) {
                ...
                domMap.canvasEdit = $("#" + sign_show);
            }

            if (!sign_clear) {
                domMap.sign_show = $("#sign_clear");
            } else {
                domMap.canvasEdit = $("#" + sign_clear);
            }

            if (!sign_ok) {
                domMap.sign_show = $("#sign_ok");
            } else {
                domMap.canvasEdit = $("#" + sign_ok);
            }
            return domMap;
        }
        ...
        function onMouseMove(evt) {
            let x = getCanvasPos(canvas, evt).x,
        ...
        //填充生成的图片
        function perview() {
            let show = domMap.sign_show[0];
            show.innerHTML = "";
            show.appendChild(convertCanvasToImg(canvas));
        }
